"""
Wellby Dataset Hybrid Efficiency Training Time Evaluation
...
output_filename = 'wellby_hybrid_efficiency_comprehensive.csv'
...
def main():
    ...
    trained_models = train_wellby_cnn_with_cv(processed_data, device)
    cnn_training_time = time.perf_counter() - cnn_training_start
    
    # Step 4: Extract features
    print("Extracting features...")
    features_list, feature_extraction_time, successful_extractions = extract_wellby_features(processed_data, trained_models)
    ...
    # Step 7: Measure inference and model sizes
    inference_times = []
    for _ in range(100):
        start_time = time.perf_counter()
        # process raw PPG and extract features from one sample

        # pass the features through the model
        _ = final_model.predict(X_scaled[:1])
    ...
    print(f"SVM Training Time:        {svm_training_time:.3f} seconds")
    ...
    print(f"Total features:          {len(all_feature_cols)} (CNN: {len(cnn_columns)}, TD: {len(td_columns)})")
