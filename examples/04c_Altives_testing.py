...
#%% Feature selection
feats = ['HR_mean','HR_std','SDNN','SDSD','meanNN','medianNN','RMSSD','meanSD','sqi']
label = 'label'
...
#%% Parameter grids (same as your LOGO code)
param_grids = {
    "Random Forest": {
        'n_estimators': [50, 100],
        'max_depth': [None, 5, 10],
...
print(f"\nNote: These are single test set evaluations (like the original paper)")
print(f"Cross-validation standard deviations available for stability analysis")
...
#%% Comparison with paper and LOGO results
print(f"\n{'='*60}")
print("COMPARISON WITH OTHER METHODS")
print(f"{'='*60}")

print("Paper Results (original children dataset):")
print("- Best Accuracy: 0.90 (RF with significant features)")
print("- Best F1-Score: 0.76 (RF with significant features)")

print(f"\nOur Results (Aktives dataset, 70/30 test set evaluation):")
if len(valid_results) > 0:
    best_model = valid_results_sorted.iloc[0]
    print(f"- Best AUC: {best_model['AUC']} ({best_model['Model']})")
    print(f"- Best Accuracy: {best_acc_model['Accuracy']} ({best_acc_model['Model']})")
    print(f"- Best F1: {best_f1_model['F1 Score']} ({best_f1_model['Model']})")

print(f"\nNote: 70/30 split uses sample-level splitting (like original paper)")
print("This is less rigorous than LOGO but allows direct comparison with paper methodology")
